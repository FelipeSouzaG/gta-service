...
export const listAllScheduleOfficer = async () => {
  try {
    const response = await fetch(`${apiUrl}/api/requests/officer/all`, {
      method: 'GET',
      credentials: 'include',
    });
    const dataRequestsOfficer = await response.json();
    return dataRequestsOfficer;
  } catch (error) {
    throw ErrorBase();
  }
};
...
export const createSchedule = async (data) => {
  try {
    const response = await fetch(`${apiUrl}/api/schedules/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify(data),
