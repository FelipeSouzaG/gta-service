...
async function requestDetails(request) {
  ...
  } else if (request.requestStatus === 'Ordem de Serviço') {
    const isOrder =
  ...
  document
    ...
    .addEventListener('click', async () => {
      ...
      if (budgetValue.value === 'Aprovado') {
        ...
        sendData.feedback = 'Orçamento Aprovado pela GTA';
      } else if (budgetValue.value === 'Reprovado') {
        sendData.budgetStatus = 'Reprovado';
        sendData.feedback = 'Orçamento Reprovado pela GTA';
      } else {
        showModalAlert(
          'Alert',
